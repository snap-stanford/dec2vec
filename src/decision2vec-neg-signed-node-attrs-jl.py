...
def objective(theta, D, D_noise, D_a, D_a_noise, graph):
	res = 0.

	for i in range(len(D)):
		# for each edge attribute
		p, p_noise = sigmoid(np.dot(theta[D[i][0]], theta[D[i][1]])), sigmoid(np.dot(theta[D_noise[i][0]], theta[D_noise[i][1]]))
		for attr in D[i][2]:
			# We use Laplace smoothing for p
			p *= binom(D[i][4], 
				float(D[i][3][attr][D[i][2][attr]] + 1) / float(D[i][4] + len(graph.edge_data[attr])),
				D[i][3][attr][D[i][2][attr]])
			p_noise *= binom(D_noise[i][4],
				float(D_noise[i][3][attr][D_noise[i][2][attr]] + 1) / float(D_noise[i][4] + len(graph.edge_data[attr])),
				D_noise[i][3][attr][D_noise[i][2][attr]])
		res += np.log(p) + np.log(1. - p_noise)
		# print '>>>', np.log(p), np.log(1. - p_noise)

	for node in D:
		distance = 0.
		for pattern_node in D_a:
			distance += cosine_distance(theta[node], theta[pattern_node])
		res -= distance

	for i in range(len(D_a)):
		res += np.log(sigmoid(np.dot(theta[D_a[i][0]], theta[D_a[i][1]])))

	for i in range(len(D_a_noise)):
		res += np.log(1. - sigmoid(np.dot(theta[D_a_noise[i][0]], theta[D_a_noise[i][1]])))
		# print np.dot(theta[D_a_noise[i][0]], theta[D_a_noise[i][1]]), sigmoid(np.dot(theta[D_a_noise[i][0]], theta[D_a_noise[i][1]]))


	return res


def objective_gradient(theta, D_map, D_noise_map, D_a_map, D_a_noise_map, graph):
	grad = dict(theta)
	...
	for pattern_node in D_a_noise_map:
		for k in range(graph.dimensions):
...
def setup_sgd(graph):
	...
	for node in graph.g.nodes_iter():
		theta[node] =  1e-10 * np.random.random(graph.dimensions)
...
def stochastic_grad_desc(obj, obj_grad, graph, alpha=0.4, iters=100, tol=1e-5, batch_size=20):
	...
	while iterator < iters:
		...
		batch_a = np.random.choice(range(len(D_a)), size=min(batch_size, len(D_a)), replace=False)
		batch_a_noise = np.random.choice(range(len(D_a_noise)), size=min(batch_size, len(D_a_noise)), replace=False)
		batch_D_a, batch_D_a_noise = [D_a[i] for i in batch_a], [D_a_noise[i] for i in batch_a_noise]
		...
		grad = obj_grad(theta, batch_D_map, batch_D_noise_map, batch_D_a_map, batch_D_a_noise_map, graph)
		...
		# print grad['10']
		if iterator % 10 == 0:
		 	curr = obj(theta, D, D_noise, D_a, D_a_noise, graph)
